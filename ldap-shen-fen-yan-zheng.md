...
|     | 本节专门针对 OpenLDAP。如果你使用其他服务器，你需要查阅该服务器的文档。 |
| --- | ----------------------------------------------------------------------- |
...
你会想要在与 LDAP 服务器的连接中要求加密；否则，你用户的密码将以明文传输，这被视为不安全。我们将要使用的工具支持两种非常相似的加密，即 SSL 和 TLS。
...
若通过 ports 安装了 OpenLDAP，在/usr/local/etc/openldap/slapd.conf 中的以下配置参数将启用 TLS：
...
这里， ssf=128 告诉 OpenLDAP 要求所有连接（包括搜索和更新）使用 128 位加密。这个参数可以根据你的站点安全需求进行配置，但很少需要弱化它，因为大多数 LDAP 客户端库都支持强加密。

cert.crt、cert.key 和 cacert.crt 文件对于客户端验证你是合法的 LDAP 服务器是必要的。如果你只是想要一个运行的服务器，可以使用 OpenSSL 创建一个自签名证书：
...
在这一点上，你应该被提示输入一些值。你可以输入任何你喜欢的值；然而，"通用名称" 值必须是 OpenLDAP 服务器的完全合格域名。在我们的情况下，以及这里的示例中，服务器是 server.example.org。不正确地设置这个值将导致客户端在建立连接时失败。这可能会导致很大的挫败感，所以请确保你密切遵循这些步骤。
...
这将创建一个自签名证书，可用于 slapd.conf 中的指令，其中 cert.crt 和 cacert.crt 是同一个文件。如果你要使用许多 OpenLDAP 服务器（用于通过 slurpd 复制），你需要查看 LDAP 生成 CA 密钥并使用它来签署单独的服务器证书上的 OpenSSL 证书。
...
|     | 重要的是你的客户端能够访问 cacert.crt，否则他们将无法连接。 |
| --- | ----------------------------------------------------------- |

|     | 有两个名为 ldap.conf 的文件。第一个是此文件，用于 OpenLDAP 库，并定义如何与服务器通信。第二个是/usr/local/etc/ldap.conf，用于 pam_ldap。 |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------- |

此时，你应该可以在客户端计算机上运行 ldapsearch -Z ； -Z 表示“使用 TLS”。如果遇到错误，则说明某些配置错误；很可能是你的证书问题。使用 openssl(1)的 s_client 和 s_server 来确保你已正确配置和签署它们。
...
对于你的用户将属于的对象类，可能需要考虑一些想法。大多数工具默认会使用 people ，如果你只想提供用于身份验证的条目，则这样使用是可以的。然而，如果你还要在 LDAP 数据库中存储用户信息，你可能会想要使用 inetOrgPerson ，它有许多有用的属性。在任何情况下，相关的模式需要加载到 slapd.conf 中。

对于本例，我们将使用 person 对象类。如果你使用 inetOrgPerson ，步骤基本相同，只是需要 sn 属性。
...
我将我的 LDAP 用户的 UID 从 10000 开始，以避免与系统账户发生冲突；你可以在此处配置任何小于 65536 的数字。
...
要将这些输入到你的数据库中，你可以使用 slapadd 或 ldapadd 在包含这些条目的文件上。或者，你可以使用 sysutils/ldapvi。

现在，客户端机器上的 ldapsearch 实用程序应返回这些条目。如果是这样，那么你的数据库已正确配置为用作 LDAP 认证服务器。

## 3. 客户端配置

客户端应该已经从配置客户端中获得了 OpenLDAP 库，但如果你要安装多个客户端机器，则需要在每台机器上安装 net/openldap26-client。
...
通过此配置，你应该能够对 LDAP 目录中的用户进行身份验证。PAM 将使用你的凭据执行绑定，如果成功，将告诉 SSH 允许访问。
...
另外，此指令在身份验证期间不会在 PAM 中进行检查，而是在帐户管理期间进行检查，因此你需要在 account 下的 PAM 文件中添加第二行。这将要求每个用户都列在组中，这不一定是我们想要的。 为了避免阻止未在 LDAP 中的用户，你应该启用 ignore_unknown_user 属性。 最后，你应该设置 ignore_authinfo_unavail 选项，以便在 LDAP 服务器不可用时不会被锁定在每台计算机上。

你的 pam.d / sshd 可能会变成这样：
...
这将允许你将用户名映射到 UID 和 UID 映射到用户名。

恭喜！你现在应该拥有可用的 LDAP 身份验证。

### 3.3. 注意事项

不幸的是，在撰写本文时，FreeBSD 不支持使用 passwd(1) 更改用户密码。因此，大多数管理员需要自己实现解决方案。我在这里提供一些示例。请注意，如果你编写自己的密码更改脚本，需要注意一些安全问题；请参阅密码存储。
...
现在，你的计算机（可能还有其他服务）正在对 LDAP 服务器进行身份验证，因此至少需要像正常服务器上的 /etc/master.passwd 一样对此服务器进行保护，甚至可能需要更好，因为破解或被入侵的 LDAP 服务器会破坏每个客户端服务。

请记住，本节内容并不详尽。你应该持续审查你的配置和流程以进行改进。
...
另外，你需要阻止用户更改他们自己某些属性。默认情况下，用户可以更改任何属性（除了 LDAP 模式本身禁止更改的属性），例如 uidNumber 。要关闭此漏洞，请修改上述内容为
...
假设你希望你的 IT 部门能够更改用户的主目录，但你不希望所有人都能够添加或删除用户。这样做的方法是为这些管理员添加一个组：
...
默认情况下，OpenLDAP 将以存储其他数据的方式存储 userPassword 属性的值：明文。大多数时候它是 base64 编码的，这提供了足够的保护来防止诚实的管理员知道你的密码，但除此之外没有什么别的保护。

因此，将密码存储为更安全的格式（例如 SSHA，盐化的 SHA）是一个好主意。这是通过你用来更改用户密码的任何程序完成的。

## 附录 A：有用的辅助工具

如果你有许多用户且不想手动配置所有内容，可能有一些其他程序很有用。

security/pam_mkhomedir 是一个始终成功的 PAM 模块；其目的是为那些还没有家目录的用户创建家目录。如果你有数十个客户端服务器和数百名用户，使用此方法并设置骨架目录要比准备每个家目录容易得多。

sysutils/ldapvi 是一个使用类似 LDIF 语法编辑 LDAP 值的实用工具。目录（或目录的子部分）以 EDITOR 环境变量选择的编辑器呈现。这使得在目录中进行大规模更改变得容易，而无需编写定制工具。

security/openssh-portable 具有联系 LDAP 服务器以验证 SSH 密钥的能力。如果你有许多服务器且不想在所有服务器上复制你的公钥，这非常方便。
...
这将是你的根 CA 密钥和证书。你可能希望加密密钥并将其存储在一个阴凉而干燥的地方；任何能访问到它的人都可以冒充你的 LDAP 服务器之一。

接下来，使用上述的头两个步骤创建一个名为 ldap-server-one.key 的密钥和一个名为 ldap-server-one.csr 的证书签名请求。若你使用 root.key 签署签名请求，你将能够在你的 LDAP 服务器上使用 ldap-server-one.*。

|     | 不要忘记在生成证书签名请求时使用完全合格的域名作为“通用名称”属性；否则客户端将拒绝与你建立连接，并且诊断可能非常棘手。 |
| --- | ---------------------------------------------------------------------------------------------------------------------- |
...
最后，为了使客户端信任你所有的服务器，请将 root.crt（证书，而非密钥！）分发给每个客户端，并在 ldap.conf 中的 TLSCACertificateFile 指令中指定它。
