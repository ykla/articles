...
* [ ] 新增概念 “datastore” 与命令集 `vm datastore`。可创建多个虚拟机存储位置。guest 可在特定数据存储上创建，我们会在所有位置中查找用户指定的 guest。
...
当前计划是将现有代码库（截至 0.12.5）稳定为 1.0 版本，待确认无明显错误即可发布。我对当前功能集很满意。已测试 FreeBSD / Linux / NetBSD / OpenBSD / Windows 多种 guest，运行正常。我尝试设置了 5 个在主机启动时自动启动的 guest，均无问题。
...
使用 FreeBSD 的 `default.conf` 模板创建一个新的 guest，运行安装程序并连接到其控制台。此时可像普通系统一样进行安装。如果在安装命令前添加选项 `-f`，guest 将直接在你的终端上运行，因此不需要再执行 console 命令（请注意，选项 `-f` 仅在 `vm-bhyve-0.12.5+` 中可用）。
...
所有模板均存放在目录 `$vm_dir/.templates` 下，文件名应为 `[my-template-name].conf`。你可以创建任意数量的模板，并在创建 guest 时指定，例如：
...
默认情况下，`vm-bhyve` 使用 `cu`/`nmdm` 作为控制台会话管理工具。如果你希望虚拟机运行在 `tmux` 会话中，从 v1.1+ 开始支持此功能。只需在 `$vm_dir/.config/system.conf` 中设置选项 `console`：
...
该文件列出了已经测试过的 guest，并展示了安装器和 guest 启动时可使用的配置选项。目标是建立一个配置列表，供大家在自己的模板中使用。在大多数情况下，guest 都是使用其默认安装选项进行测试的。
...
建议至少为  Windows 准备 1G 内存，磁盘模拟需要使用 ahci-hd。

```ini
guest="windows"
uefi="yes"
disk0_type="ahci-hd"
# 若 Windows 版本小于 10，需要设置 sectorsize
...
* 对于 UEFI，安装盘需要是 `.img` 格式，而不能是 `.iso`。
* 使用命令：`sudo vm iso https://cdn.openbsd.org/pub/OpenBSD/7.6/amd64/install76.img`

### Alpine Linux

Alpine 的特殊之处在于安装 CD 上的文件针对不同的 Alpine 版本有不同的后缀。基本文件名为 `vmlinuz-[suffix]` 和 `initramfs-[suffix]`。请相应调整你的 vm `.conf` 文件：
...
请注意，CentOS 的 kernel 和 initramfs 文件包含内核版本号，如果系统没有内核 `2.6.32-573.el6`，需要修改配置。
...
当从模板创建 guest 时，会为模板中指定的每个磁盘创建一个磁盘镜像。如果希望在 guest 创建后添加磁盘，可以使用命令 `vm add`，或者先创建磁盘镜像，再手动更新配置文件。
...
可自定义磁盘镜像的路径。磁盘可以是稀疏文件、ZVOL，甚至是 `/dev/` 下的真实磁盘：
...
另外，可在 `disk1_name` 后追加 `,ro` 来设置为只读。
...
通过指定自定义设备，你可以通过 `rc.conf` 自行配置该设备。请注意，我省略了配置选项 `switch`。如果指定了，tap 设备会自动附加到虚拟交换机。如果你已按需求配置了自定义设备，可以省略 switch 设置。
...
在运行 `vm-bhyve` 时，该目录用于 ISO 文件和核心配置数据，同时也成为虚拟机的默认数据存储。如果列出数据存储，将显示如下信息：
...
在示例模板中，每个 guest 都有一个接口连接到虚拟交换机 `public`。使用这个名称没有特殊原因，你可以为交换机取任意名字：
...
通过指定 Windows ISO 文件正常启动安装。在安装模式下运行时，`vm-bhyve` 会等待 VNC 客户端连接后再启动 guest，以便捕捉 Windows 的选项 “Boot from CD/DVD”（从 CD/DVD 启动）。在此期间，`vm list` 中显示 guest 为锁定状态。
...
虽然 网络适配器 `e1000` 能直接使用并提供网络访问，但建议尽可能使用设备 `virtio-net`。安装驱动的方法有几种：
...
大约每六个月发布一次稳定版本，并在发布后享有一年的支持。建议访问官方 [OmniOSce 下载页面](https://omniosce.org/download.html)。
...
OmniOSce 支持 `virtio-net` 驱动，相比 `e1000` 提供了更好的性能：
...
注意：如果你的防火墙是 `ipfw`/`firewall`，那么这些设置适用；如果你使用 `pf`，也许需要不同的设置。
...
按任意键后，从 GRUB 菜单按 `c` 进入 grub 命令行。输入 `ls` 查看可用设备，对于 `alpine-virt-3.13.2-x86_64.iso`，结果为：`(cd0) (cd0,msdos2) (hd0) (host)`
...
vm-bhyve 支持为虚拟机提供基础的 cloud-init 配置。可通过 `vm create` 命令的选项 `-C` 启用。你还可以使用选项 `-k <file>` 将公钥注入虚拟机。使用 cloud-init 需要安装软件包 `cdrkit-genisoimage`：
