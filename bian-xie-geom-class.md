...
对于更多的调试，你还应该包括 WITNESS 支持，这将在锁定错误时通知你：
...
另一个方便之处是启用内核调试器，这样你就可以在内核发生 panic 时对其进行检查。为此，请在内核配置文件中输入以下行：
...
为了使其工作，你可能需要设置一个 sysctl（如果默认情况下不打开）：
...
写入内核核心转储很慢，需要很长时间，因此，如果你有大量内存（>256M）和大量的 panic ，坐在一边等待完成可能会很沮丧（首先将其写入交换空间，然后将其重新定位到文件系统）。因此，通过/boot/loader.conf 可调节限制系统将使用的 RAM 量是很方便的：

```
  hw.physmem="256M"
```

如果 panic 频繁发生且文件系统较大（或者你根本不信任 softupdates+后台 fsck），建议通过/etc/rc.conf 变量关闭后台 fsck：
...
为了创建一个新的 GEOM 类，必须在任意用户可访问的目录下创建一个空的子目录。你不必在/usr/src 下创建模块目录。
...
FreeBSD 内核中的互斥锁（参见 mutex(9)）与其更常见的用户空间表亲有一个区别 - 在持有互斥锁时代码不能休眠）。如果代码需要频繁休眠，sx(9)锁可能更合适。另一方面，如果你几乎所有操作都在单个线程中完成，你可能根本不需要互斥锁。
