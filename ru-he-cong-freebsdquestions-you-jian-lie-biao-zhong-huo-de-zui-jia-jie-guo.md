...
不可避免地，由于这两个群体的观点不同，会产生一些摩擦。新手指责黑客傲慢、自以为是、没有帮助，而黑客则指责新手愚蠢、无法理解简单的英语、并期待所有问题都能不劳而获。当然，这些指责中确实有些道理，但大多数情况下，这些观点源于一种挫败感。
...
- 你需要提供的许多信息是程序的输出，例如 [dmesg(8)](https://man.freebsd.org/cgi/man.cgi?query=dmesg&sektion=8&format=html) 命令的输出，或是通常出现在 **/var/log/messages** 中的控制台消息。不要通过手动输入再次复制这些信息；这既麻烦又容易出错。为了发送日志文件内容，你可以复制文件并使用编辑器只保留相关信息，或者直接将内容复制粘贴到你的邮件中。对于类似 [dmesg(8)](https://man.freebsd.org/cgi/man.cgi?query=dmesg&sektion=8&format=html) 这样的程序输出，你可以将输出重定向到一个文件中并附上该文件。例如，
...
2. 是否已经有人回答了这个问题？检查是否有人已经回答，可以通过按主题对收件箱中的邮件进行排序来完成：希望你能看到问题以及所有回答，全部在一起。如果有人已经回答了问题，这并不意味着你不能再提供一个答案，但最好先阅读其他人的回答。
3. 你是否有其他贡献，超出已经给出的答案？一般来说，“是的，我也遇到过”这样的回答帮助不大，尽管也有例外。例如，某人介绍了他们遇到的问题，他们不知道是自己的错还是硬件或软件有问题。如果你发送了“我也遇到过”的回答，应该同时提供其他相关信息。
4. 你是否确定理解了问题？提问的人经常会感到困惑或表达不清。即使你对系统有最好的理解，仍然很容易给出一个不能回答问题的回复。这没有帮助，反而会让提问者比以往更加沮丧或困惑。如果没有其他人回答，并且你自己也不太确定，可以请求更多的信息。
...
11. 如果提问者没有遵循格式规定（比如行太长、主题不合适），请*修正*。如果是错误的主题行（如“HELP!!??”），请将主题行改为（例如）“Re: 同步 PPP 的困难（原主题：HELP!!??）”。这样其他人跟踪邮件线程时会更容易。
    在这种情况下，适当地说明你做了什么以及为什么这样做是合适的，但请尽量避免显得无礼。如果你发现自己无法在不显得无礼的情况下回答，可以选择不回答。
