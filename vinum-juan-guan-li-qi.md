...
**vinum** 维护一个 *配置数据库*，介绍了系统中已知的对象。用户最初通过一个或多个配置文件使用 [gvinum(8)](https://man.freebsd.org/cgi/man.cgi?query=gvinum&sektion=8&format=html) 创建配置数据库。**vinum** 在每个受其控制的硬盘 *设备* 上存储其配置数据库的副本。该数据库在每次状态变化时更新，以便在重启时准确恢复每个 **vinum** 对象的状态。
