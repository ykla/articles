...
MFC 代表 `Merge From CURRENT`，即从我们的 -CURRENT 开发分支合并经过测试的更改到 -STABLE 分支。项目政策要求任何更改必须首先应用到主干分支，然后在经过 -CURRENT 用户的充分外部测试后再合并到 -STABLE 分支（开发人员被要求在提交到 -CURRENT 之前进行广泛测试，但由于一个人无法覆盖通用操作系统的所有使用情况，因此无法保证所有使用场景都能被测试）。最短 MFC 期限为 3 天，通常仅用于微小或关键的 bug 修复。
