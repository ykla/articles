...
一份（相当）完整的 vm-bhyve 当前支持功能列表。除非另有说明，所有功能均适用于所有类型的 guest。

* 简单的配置文件，可轻松手动编辑
* FreeBSD guest（包括基于 FreeBSD 的软件，如 pfSense）
...
* 支持稀疏文件、zvol、sparse-zvol 或自定义磁盘设备
...
* [ ] `graphics_vga=on|off|io` 选项，用于控制 `vga` fbuf 参数。在 UEFI 下的 OpenBSD 指令中使用此设置。
* [ ] 如果安装介质文件名不以 `.iso` 结尾，vm-bhyve 现在会自动切换为 `ahci-hd`，并将此设备设置为只读。

### 版本 1.2（当前 FreeBSD Ports 版本）


这是当前的开发版本，包含以下新特性：

* [ ] 支持在单个控制器上最多使用 32 个 AHCI 设备。此前，bhyve 为每个设备添加一个新的 AHCI 控制器，这在 UEFI guest 中极大限制了可附加设备数量。（仅限 FreeBSD 12）
* [ ] 新增 `vm set` 与 `vm get` 命令以管理全局配置变量。目前仅支持 `console` 变量，但这为未来更多基本功能的可配置化铺平了道路。
* [ ] “交互模式”支持：guest 在 `tmux` 会话中启动。与普通模式不同，`tmux` 会话不会分离，这提供了类似前台模式的体验；优势在于 `tmux` 会话仍可手动分离，让 guest 在后台继续运行。
* [ ] 命令部分匹配支持，例如可使用 `vm sw l` 列出虚拟交换机。
* [ ] 1.2 默认对所有 guest 使用 UTC 时钟，除非明确禁用。这种行为更一致、更可预期。
* [ ] “media” 数据存储支持。如果你有一个存放 ISO 镜像的目录，就没必要将它们复制到 `$vm_dir/.iso`。现在你可以直接将该目录添加为媒体数据存储，我们会在此目录中查找 ISO 文件。运行安装命令时也可直接使用 ISO 的完整路径，我们会在当前目录中查找。
* [ ] `vm start` 命令现在可以同时启动多个 guest。
* [ ] 虚拟交换机重构。桥接现在以交换机命名，代码模块化。
...
* [ ] 新增 “datastore” 概念与 `vm datastore` 命令集。允许创建多个虚拟机存储位置。guest 可在特定数据存储上创建，我们会在所有位置中查找用户指定的 guest。
* [ ] 将“全局”配置移入 `$vm_dir/.config/system.conf`。之前存在独立的 `switch` 与 `datastore` 配置文件，现在已合并。我们添加了新的 `__config` 函数用于加载与访问该文件，无需频繁调用 `sysrc`。
* [ ] 移除了 `guest` 配置项。用户需确保为需要的 guest 指定 `loader="grub"` 或 `loader="bhyveload"`。
...
当前计划是将现有代码库（截至 0.12.5）稳定为 1.0 版本，一旦确认无明显错误即可发布。我对当前功能集很满意。已测试多种 FreeBSD / Linux / NetBSD / OpenBSD / Windows guest，运行正常。我尝试设置了 5 个在主机启动时自动启动的 guest，均无问题。

截至 4 月 15 日，当前代码已升级为 1.0-beta。我计划保留一到两周，此期间只递增修订号/构建号（现在为独立整数）。之后我将发布 1.0 并提交 Port 更新。任何功能变更将纳入 1.1，并暂时仅在 GitHub 发布。我不希望频繁提交 Port 更新到 FreeBSD；想要最新功能的用户可从 GitHub 获取，其他用户则可在 Ports 获取稳定的 1.0。

该版本已分支为 1-0.stable。一旦确认脱离 beta 状态，Ports 将更新此版本。版本号简化为 1.0，并附加构建号。如需修复，将在此分支完成，并在必要时以 1.0-pX 形式更新 Ports。
...
* [ ] 旧版本中，若启动命令错误，`grub-bhyve` 可能挂起并占用 100% CPU。此问题已解决。
* [ ] 配置文件中不再需要 `boot` 命令，简化了配置。


### 版本 0.10（已完成）

完全重写了 `grub-bhyve` 支持。用户可通过 `grub_installX` 与 `grub_runX` 选项完全控制传递给加载器的启动与安装命令。
...
如果需要支持 Linux grub，可安装 `grub2-bhyve` 包；如需运行包括 Windows 在内的 UEFI 启动系统，请安装 UEFI 相关 Port。

**第 2 行**
为虚拟机创建一个数据集。如果不使用 ZFS，只需创建一个普通目录即可。
...
**第 7–8 行**
创建名为 “public” 的虚拟交换机，并将你的网络接口附加到其中。将 `em0` 替换为实际连接网络的接口。

**第 9 行**
从 FTP 站点下载 FreeBSD 镜像。

**第 10–12 行**
使用 FreeBSD 的 `default.conf` 模板创建一个新的 guest，运行安装程序并连接到其控制台。此时可像普通系统一样进行安装。如果在安装命令前添加 `-f` 选项，guest 将直接在你的终端上运行，因此不需要再执行 console 命令。（请注意，`-f` 选项仅在 `vm-bhyve-0.12.5+` 中支持。）
...
所有模板均存放在 `$vm_dir/.templates` 目录下，文件名应为 `[my-template-name].conf`。你可以创建任意数量的模板，并在创建 guest 时指定，例如：

```
vm create -t my-template-name myguest
```

如果在创建 guest 时未指定模板名称，则默认使用 `default.conf`。
...
**第 7-8 行**
第一块网络适配器的选项。如果不需要网络，可以删除这些选项。目前唯一支持的接口类型是 `virtio-net`。该接口会自动连接到 `public` 虚拟交换机（如果存在）。

## 使用 tmux

**Oct 3, 2022**

默认情况下，`vm-bhyve` 使用 `cu`/`nmdm` 作为控制台会话管理工具。如果你希望虚拟机运行在 `tmux` 会话中，从 v1.1+ 开始已支持此功能。只需在 `$vm_dir/.config/system.conf` 中设置 `console` 选项：
...
该文件列出了已经测试过的 guest，并展示了允许安装器和 guest 启动时使用的配置选项。目标是建立一个配置列表，供大家在自己的模板中使用。在大多数情况下，guest 都是使用其默认安装选项进行测试的。

如果你成功使用了这里未列出的 guest 操作系统，请告知我，以便更新列表。

请注意，我在这些示例中省略了网络和磁盘设置，因为所有 guest 的格式都是相同的。不同配置请参见网络或磁盘示例页面。
...
如果你使用的是 `vm-bhyve-0.11` 或更新版本，grub guest 不再需要 `boot` 命令，因此安装和运行时的 guest 配置文件中可以去掉该选项。

在 `vm-bhyve-0.12.5` 中，如果分区 1（msdos1/openbsd1）正确，任何 `grub-bhyve` guest 都不需要 `grub_run_partition` 选项。
...
建议 Windows 至少使用 1G 内存，磁盘模拟需要使用 ahci-hd。

```
guest="windows"
uefi="yes"
disk0_type="ahci-hd"
# 如果 Windows 版本小于 10，需要设置 sectorsize
...
以下示例针对 OpenBSD 6.2 amd64，因为安装命令包含版本号和架构。我已成功运行 5.2，其他版本只要更新安装命令中的版本号也应可用。i386 guest 同样可用，但需更新安装命令。
...
resflash 是用于 USB 闪存的 OpenBSD 镜像，也适合作为虚拟网络设备。以下示例为 resflash 20171202_1952 amd64。
...
* 安装器默认会选择错误的根磁盘，请确保选择正确的磁盘（sd1）。
* 安装完成后，请执行 `vm poweroff openbsd-vm`，否则 VM 会重新启动到安装盘。
* 对于 UEFI，安装盘需要是 `.img` 格式，而不是 `.iso`。
* 使用命令：`sudo vm iso https://cdn.openbsd.org/pub/OpenBSD/7.6/amd64/install76.img`

### Alpine Linux

Alpine 的一个特殊之处在于安装 CD 上的文件针对不同的 Alpine 版本使用不同的后缀。基本文件名为 `vmlinuz-[suffix]` 和 `initramfs-[suffix]`。请相应调整你的 vm `.conf` 文件：
...
请注意，CentOS 的 kernel 和 initramfs 文件包含内核版本号，如果系统不包含 `2.6.32-573.el6` 内核，需要修改配置。
...
基于内核 `2.6.32-431.el6.x86_64`。如果使用不同版本内核，请相应修改配置。如果不确定内核版本，可在不指定 grub 命令的情况下启动 guest，然后使用 `vm console` 访问 bootloader，在 bootloader 中查找正确内核（`ls (hd0,1)/`）。
...
如果需要 Ubuntu 14.04，还需添加以下内容，否则会停留在 grub 提示符：
...
OpenWRT x86/64 可以使用 uefi-csm 固件运行。
下面是一个配置示例。只需下载最新的磁盘镜像并放入 vm 目录。
...
* 下载 x86/64 ext4 efi 镜像，用 gzip 解压，然后复制到 VM 目录，并重命名为 `disk0.img`，覆盖已创建的 `disk0.img`。
...
当从模板创建 guest 时，会为模板中指定的每个磁盘创建一个磁盘镜像。如果希望在 guest 创建后添加磁盘，可以使用 `vm add` 命令，或者先创建磁盘镜像，再手动更新配置文件。
...
允许指定磁盘镜像的自定义路径。磁盘可以是稀疏文件、ZVOL，甚至是 `/dev/` 下的真实磁盘：
...
首先为磁盘镜像创建新的 ZVOL。如果不希望 ZVOL 是稀疏的，请去掉 `-s` 选项：
...
可选地，在 `disk1_name` 后追加 `,ro` 来设置为只读。
...
请注意，在首次运行时，如果网络接口没有 MAC 地址，`vm-bhyve` 会自动为其分配一个静态 MAC 地址，并写入配置文件。
...
`bhyve` hypervisor 仅接受以 `58:9c:fc` 开头的自定义 MAC 地址，否则将无法启动（无论 guest OS 是什么），且不会给出有用的错误信息。

### 自定义网络设备

默认情况下，`vm-bhyve` 会为每个接口创建动态 tap 设备。如果需要进行复杂配置，需要手动设置网络，这可能会成为问题，因为设备在启动 guest 之前并不存在。

通过指定自定义设备，你可以通过 `rc.conf` 自行配置该设备。请注意，我省略了 `switch` 配置选项。如果指定了，tap 设备会自动附加到虚拟交换机。如果你已按需求配置了自定义设备，可以省略 switch 设置。
...
运行 `vm-bhyve` 时，该目录用于 ISO 文件和核心配置数据，同时也成为虚拟机的默认数据存储。如果列出数据存储，将显示如下信息：
...
创建虚拟机时，默认会使用默认数据存储。但可以通过 `-d` 选项指定其他数据存储：

```
# vm create -d ssd -t centos7 -s 50G centos-ssd
```

`list` 和 `info` 命令也已更新，可显示数据存储信息：
...
请注意，guest 名称仍需在所有数据存储中唯一。如果存在多个相同名称的 guest（例如添加一个已包含 guest 的数据存储），`vm-bhyve` 命令会按添加顺序搜索数据存储，只操作找到的第一个 guest。
...
### 创建一个名为 `public` 的简单交换机

在示例模板中，每个 guest 都有一个接口连接到 `public` 虚拟交换机。使用这个名称没有特殊原因，你可以为交换机取任意名字：
...
上述命令创建了虚拟交换机，并将物理接口 `em0` 连接到它。如果 `em0` 能访问 Internet，那么连接到该交换机的 guest 也能访问 Internet。
...
如果无法删除虚拟交换机，请检查该交换机是否有对应的 bridge。通常应为 `vm-<switch_name>` 格式，或者如果名称包含 `-`，则为 `bridge<#>`。如果不符合，需要先删除相关虚拟交换机配置，再重新创建交换机。
...
不幸的是，从 v1.2 起，`vm-bhyve` 已移除了内部 NAT 配置。作为一个 shell 脚本，之前依赖配置外部系统（如 pf 和 dnsmasq）来提供 NAT 功能。有些用户希望使用其他工具或防火墙，很多用户由于已有的 pf 或 dnsmasq 配置而导致 NAT 出现问题。因此，现在可以说手动配置 NAT 比尝试通过 `vm-bhyve` 启用 NAT 并手动安装/调整生成的配置更容易且出错率更低。
...
为 guest 提供 DHCP 服务可以使用 `dnsmasq`（需通过 Ports 安装）。基于上述虚拟交换机 `public` 和 `192.168.8.x` 网络范围，一个简单配置示例如下：
...
`vm-bhyve` 内置支持 `grub2-bhyve` 软件，它能够启动大多数可由 GRUB2 启动的 guest。本页介绍可在 guest 配置文件中设置的各种配置选项，以控制 `grub2-bhyve` 及其效果。
...
例如，CentOS 7 使用 `/grub2/grub.cfg`，文件名未变，只需指定新目录：
...
如果需要多个命令，可增加行并依次编号为 `grub_run1`、`grub_run2` 等。指定命令后，vm-bhyve 会生成自定义 grub.cfg 文件。启动 guest 时，会显示包含单个 `guestname (bhyve run)` 选项的自定义 grub 菜单，该选项默认 3 秒后选择（可用 `loader_timeout="X"` 配置更改超时）。

对于大多数 Linux guest，最好在 guest 内安装 GRUB2，以便 guest 自行管理配置文件。内核升级可能需要修改启动命令，如果手动写入 guest 配置文件，未更新命令将导致无法启动。

### 安装器命令

正常情况下，`grub2-bhyve` 会在 CD 的 `/boot/grub/grub.cfg` 查找 grub 配置文件，如找不到，则进入 `grub>` CLI。我们未提供配置选项指定其他位置，但提供了自定义启动命令的功能。例如 CentOS 6 样例模板的 ISO 安装命令如下：
...
如果计划运行 Windows，强烈建议使用 FreeBSD 11 及以上版本，并启用 UEFI 图形支持。
...
使用 Windows 模板创建一个 guest。默认配置为 2 个 CPU 和 2GB 内存，同时使用 Windows 原生支持的 `e1000` Intel 网络适配器。如果需要修改配置选项，可使用 `vm configure guest` 命令。

> 注意：如果运行的是 Windows 10 以前版本，需要使用 `disk0_opts="sectorsize=512"` 将磁盘扇区大小设置为 512。安装 Microsoft SQL Server 时也必须设置扇区大小为 512。
...
通过指定 Windows ISO 文件正常启动安装。在安装模式下运行时，`vm-bhyve` 会等待 VNC 客户端连接后再启动 guest，以便捕捉 Windows 的 “从 CD/DVD 启动” 选项。在此期间，`vm list` 中显示 guest 为锁定状态。
...
虽然 `e1000` 网络适配器可以直接使用并提供网络访问，但建议尽可能使用 `virtio-net` 设备。安装驱动的方法有几种：
...
部分 Windows 版本（多数桌面版）不支持超过一个物理 CPU。默认情况下，bhyve 会将每个虚拟 CPU 配置为一个独立 package。

可通过修改 `hw.vmm.topology.cores_per_package` sysctl 来创建每个 CPU 的多个核心而不是多个 package。例如，将其设置为 4，将使 8 vCPU 的 guest 配置为 2 x 4 核 package。

该 sysctl 必须在 `/boot/loader.conf` 中设置，并重启生效。
...
从 FreeBSD 12.1R 起，bhyve 支持 NVMe 模拟。在 vm-bhyve 配置中，可使用以下选项：
...
甚至可以将 guest 安装到物理 NVMe 磁盘，无需虚拟磁盘。例如：
...
目前 NVMe 启动支持 Windows 8.1 及以后的版本。如果要从 NVMe 磁盘启动 Windows 7，请按以下步骤操作：

1. 使用 ahci-hd 控制器安装 Windows 7 guest，按常规流程进行。
2. 安装后，添加额外 disk1.img 并使用 NVMe 控制器。
...
5. 关闭 guest，移除 AHCI 控制器和 disk1.img，仅保留 NVMe 控制器与 disk0.img，然后再次启动。此时 Windows 7 guest 的设备管理器中将只显示 NVMe 控制器，不再存在 AHCI 控制器。
...
OmniOS 的开发已在 OmniOSce 中继续，由 OmniOS 社区和基金会提供支持。现在不再需要修改安装介质，这在旧版本的 OmniOS 中是必须的（可查看此页面的旧版本）。

### 下载最新版本

稳定版本大约每六个月发布一次，并在发布后支持一年。建议访问官方 [OmniOSce 下载页面](https://omniosce.org/download.html)。
...
OmniOSce 支持 `virtio-net` 驱动，相比 `e1000` 提供更好的性能：
...
从最新版本起，不再必须使用串行控制台。你可以通过 UEFI loader 使用 VNC 查看器。不过，如果你偏好串行控制台，可以按如下方式启用：

安装程序的最后一步允许获取 shell 提示以自定义安装，使用菜单项：**Shell (for post-install ops on /mnt)**。
...
如果你有配置 `lagg`（网络冗余），请使用例如 `lagg0` 作为接口：
...
注意：如果你使用的是 `ipfw`/`firewall` 作为防火墙，这些设置适用；如果使用 `pf`，可能需要不同的设置。
来源：[FreeBSD 论坛](https://forums.freebsd.org/threads/bhyve-and-firewall-on-host.75089/)

要在开机时生效，请将所需值追加到 `/etc/sysctl.conf` 中，但省略 `sysctl ` 这一部分。在我的例子中，我添加了：
...
接下来，为了能够安装和运行 Alpine Linux，先用 `alpine` 模板创建虚拟机：
`vm create -t alpine -s 5G alpine`

这一步会运行失败，但允许你进入 grub 命令行。在这里你可以找到所选 Alpine ISO 中实际的 Linux 内核名称；不同发行版名称不同。vm-bhyve 提供的模板使用的是 `vmlinuz-vanilla` 和 `initramfs-vanilla`，这是与发行版相关的部分。
...
按任意键后，从 GRUB 菜单按 `c` 进入 grub 命令行。
输入 `ls` 查看可用设备，对于 `alpine-virt-3.13.2-x86_64.iso`，结果为：
...
接下来，编辑该虚拟机的配置文件：`vm config alpine`（将 `alpine` 替换为你的虚拟机名称），或者复制模板创建自己的模板。
对于本例中的 ISO，需要将所有 `vanilla` 替换为 `virt`，即将 `/boot/vmlinuz-vanilla` 替换为 `/boot/vmlinuz-virt`，`/boot/initramfs-vanilla` 替换为 `/boot/initramfs-virt`。

如果使用 `vm config alpine`，只需重新运行：
`vm install alpine alpine-virt-3.13.2-x86_64.iso`
然后 `vm console alpine`

或者如果你创建了新模板，则创建新虚拟机，再运行安装。

登录用户名为 `root`（无密码），此时系统消息会提示运行 `setup-alpine`。
这个命令才是真正安装 Alpine 的步骤，在此之前网络不会工作。
...
可以通过 `sysutils/uefi-edk2-bhyve` Port 安装 UEFI 固件。依赖较多，建议使用 pkg 安装：

```sh
# pkg install uefi-edk2-bhyve
```

> 更新：现在 `pkg install sysutils/bhyve-firmware` 会自动安装 `edk2-bhyve` 和 `uefi-edk2-bhyve-csm`。原 `sysutils/uefi-edk2-bhyve` Port 已删除。
...
在 `/etc/default/grub.d/99-bhyve.cfg` 创建一个文件，内容如下：
...
按照 [示例模板](https://github.com/churchers/vm-bhyve/pull/387) 和 [Red Hat 官方文档](https://access.redhat.com/articles/7212#config8) 操作，作为 root 执行以下命令：
...
理想情况下应使用免密码的密钥认证（passwordless key-auth），尽管不是严格必要的。
...
你可以使用云镜像来创建虚拟机。`vm img` 命令会将镜像下载到 datastore，并在需要时解压（支持 `.xz`、`.tar.gz` 和 `.gz` 文件）。镜像应为 RAW 或 QCOW2 格式。使用此功能需要安装 `qemu` 或 `qemu-devel` 包：
...
vm-bhyve 支持为虚拟机提供基础的 cloud-init 配置。可以通过 `vm create` 命令的 `-C` 选项启用。你还可以使用 `-k <file>` 选项将公钥注入虚拟机。使用 cloud-init 需要安装 `cdrkit-genisoimage` 包：
