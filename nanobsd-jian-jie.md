...
* 运行时一切为只读 - 即使在系统非正常关机后，拔掉电源也安全，不需要运行 [fsck(8)](https://man.freebsd.org/cgi/man.cgi?query=fsck&sektion=8&format=html)。
...
* 由于 NanoBSD 主要设计用于构建嵌入式设备的系统映像，假设使用的存储媒体将相对较小。因此，所布置的文件系统配置为具有较小的块大小（4KB）和较小的碎片大小（512B）。可以通过 `NANO_NEWFS` 变量修改文件系统的配置选项，但语法必须遵循 [newfs(8)](https://man.freebsd.org/cgi/man.cgi?query=newfs&sektion=8&format=html) 命令格式。此外，默认情况下，文件系统启用了软更新。可以查看 [FreeBSD Handbook](https://docs.freebsd.org/en/books/handbook/) 了解更多相关信息。
